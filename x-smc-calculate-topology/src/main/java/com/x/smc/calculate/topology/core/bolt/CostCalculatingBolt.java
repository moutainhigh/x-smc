package com.x.smc.calculate.topology.core.bolt;
...
import com.x.storm.message.MappingRule;
import com.x.storm.message.MessageParser;
...
import com.x.smc.calculate.topology.core.proxy.CalculateProxy;
import com.x.smc.calculate.topology.core.util.LoadConfUtil;
...
//                    .getCacheClient(SmcCacheConstant.NameSpace.CAL_COMMON_CACHE);
            String counter = String.valueOf(RedisUtil.getCacheClient().hincrBy(SmcCacheConstant.Cache.COUNTER,
                    bsn, 1));
            LOG.info("详单算费累加器[key1:" + SmcCacheConstant.Cache.COUNTER + ", key2:" + bsn
                    + ", value = " + counter + "]");
            String original = StringUtils.defaultString(RedisUtil.getCacheClient().hget(
