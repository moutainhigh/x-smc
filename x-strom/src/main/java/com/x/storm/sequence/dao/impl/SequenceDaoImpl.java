package com.x.storm.sequence.dao.impl;
...
public class SequenceDaoImpl implements ISequenceDao {
    ...
    public void modifySequence(String sequenceName, long nextval) {
        ...
        if (sequence != null) {
            throw new RuntimeException("create sequence,the sequence name is " + sequenceName
                    + "have exists!");
        }
        Connection conn = null;
        PreparedStatement ps = null;
        try {
            ...
            conn.commit();
            if (count != 1) {
                throw new RuntimeException("modify sequence failed, update data more than one!");
            }
        } catch (Exception e) {
